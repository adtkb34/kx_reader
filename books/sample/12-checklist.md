...
- [ ] `book.json` 的 `contents` 页项含 `id` + `file`，且与 frontmatter `id` 一致；组 id / 页 id 全书唯一
- [ ] 若声明 `lenses`：树节点可含 `children`；`page.lenses` 只挂**叶子**（`true` 或小节列表）；无 `correspondences`；页 frontmatter 不含 `layer` / `pair`；节点 id 非空且全书唯一
