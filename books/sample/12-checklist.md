...
- [ ] `book.json` 的 `contents.dimensions` 页项含 `id` + `file`，且与 frontmatter `id` 一致；组项含 `id` + `title`；`contents.axes` 只引用这些 id；组 / 页 id 全书唯一
- [ ] 若声明 `lenses`：`dimensions` 词表 + `axes` 树；页项 `lenses` 只挂**叶子**（`true` 或小节列表）；无 `correspondences`；页 frontmatter 不含 `layer` / `pair`；节点 id 非空且全书唯一
