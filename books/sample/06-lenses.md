...
```json
{
  ...
  "correspondences": {
    ...
    "audience": [
      {
        "tenant": "auth",
        "admin": { "page": "auth", "sections": ["email-auth", "ops-entry"] }
      },
      { "admin": "ops" }
    ]
  }
...
| 写法 | 含义 |
| --- | --- |
| 轴下多键一行 | 该主题在该轴各选项下的页；切换**该轴**时跳到目标键对应的页 |
| 目标为字符串 | 整页可见 |
| 目标为 `{ "page", "sections": ["…"] }` | 同一页，但只显示所列小节 id；列出的 h2 会带上其后更深的子标题，直到下一个同级或更高级标题 |
| 多选项指向同一 `page` | 该页同时归属多个选项（如租户与运营共用「账号登录」） |
| 轴下单键一行 | 仅归属该选项，无对侧 |
| 未出现在某轴任何行 | **该轴常显** |
...
- 同一章节 id 在同一轴内只能出现在一行 `correspondences` 里；一行内多个选项可指向同一页。
