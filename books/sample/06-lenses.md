...
维度可有层级：选中**叶子** = 显示该叶挂靠正文，并保留未挂靠内容与 **index 小节壳**（其它叶挂靠隐藏）；选中**父节点 / 轴** = **该维不筛选**（挂任意子叶的与未挂的都显示）。也可多选若干同层叶。**同一分支同时只能选一层**（父与子不可并存）；跨轴 / 不同分支互不影响。多轴时页要**同时**满足各轴约束才可见。
...
| 顶栏选中其它无表叶（如权限） | 该叶无挂靠正文 → 仍显示未挂靠与 index 壳；其它叶挂靠隐藏 |
| 页项 `lenses` 挂父节点 id | **禁止**；只允许叶子 |
| 顶栏选中父节点 / 轴节点 | **该维不筛选**：挂任意子叶的、以及未挂该轴任何子叶的都显示 |
| 顶栏选中叶子 | 该叶挂靠 ∪ 未挂靠 ∪ index 壳；其它叶挂靠隐藏。index 小节 id 即便挂在某叶上，选其它叶时壳仍在，仅挂靠正文按叶/父显隐 |
