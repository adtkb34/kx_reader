...
维度可有层级：选中**叶子** = 只显示该叶挂靠正文 ∪ **index 刻度壳**（未挂该轴的正文隐藏）；选中**父节点 / 轴** = **该维不筛选**（挂任意子叶的与未挂的都显示）。也可多选若干同层叶。**同一分支同时只能选一层**（父与子不可并存）；跨轴 / 不同分支互不影响。多轴时页要**同时**满足各轴约束才可见。
...
| 顶栏选中 `overview`（概览） | 该轴整页总览：不按小节 allowlist 收窄，也不按整页层隐藏其它叶的页 |
