...
title: 阅读透镜
---
本书顶栏的 **场景 / 实现** 就是透镜：同一本书、两类阅读任务，目录与翻页只展示当前类型。用下拉框切换。透镜的 **id 与显示名由每本书自己声明**，不是全局固定枚举——例如 Now Order 用「场景 / 界面」。
...
书架上的 Now Order：顶栏「场景 / 界面」；壳一次见 [结构思想](02-structure.md#shell-once)，活样在「壳与风格」。清单写法见实现透镜的 [book.json](03-format.md#manifest-tree)。
