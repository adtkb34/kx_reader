...
维度可有层级：选中**叶子** = 只显示挂在该叶上的内容（未挂该轴任何子叶的隐藏）；选中**父节点 / 轴** = **该维不筛选**（挂任意子叶的与未挂的都显示）。也可多选若干同层叶。**同一分支同时只能选一层**（父与子不可并存）；跨轴 / 不同分支互不影响。多轴时页要**同时**满足各轴约束才可见。
...
| 顶栏选中其它无表叶（如权限） | 该叶下无挂靠内容 → 小节都不显示（含未挂任何子叶的） |
| 页项 `lenses` 挂父节点 id | **禁止**；只允许叶子 |
| 顶栏选中父节点 / 轴节点 | **该维不筛选**：挂任意子叶的、以及未挂该轴任何子叶的都显示 |
| 顶栏选中叶子 | 只显示挂在该叶上的小节；未挂该轴任何子叶的隐藏 |
